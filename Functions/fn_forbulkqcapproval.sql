CREATE OR REPLACE FUNCTION fn_forbulkqcapproval(in nodetype int4, in activity int4, in types int4, in entervalue text, in fromqcrid int4, in toqcrid int4, in approvelog jsonb, in approveremark text)
...
insert into assignmentpreelogs(fntext,created) select 'fn_forbulkqcapproval'||$1::text||'/'||$2::text||'/'||$3::text||'/'||$4::text||'/'||$5::text||'/'||$6::text||'/'||$7::text||'/'||$8::text,now();
...
CREATE TEMP TABLE IF NOT EXISTS tmp_enterid_data ( enterid INT ) ON COMMIT DROP; 

INSERT INTO tmp_enterid_data (enterid)
SELECT
    (x->>'serialNo')::INT
FROM jsonb_array_elements(entervalue::jsonb) x;


if $3=1 then

    select * into rlid from tmp_enterid_data ;
...
begin
    ...
    if $5=1 and $6=2 and $2=71 then 
    begin

            UPDATE tblresponselogs r
            SET responsestatusid = 5,nextapproverroleid = 78, updated = true
            WHERE responselogid in ( rlid ) and activityid=$2 and r.nodetype=$1;
    ...
    begin
          	...
          	Select distnid into distrid from tblresponselogs where responselogid in ( rlid ) ;

         	select max(surveydetailsid) into maxsdid from tblsurveydetaails where responselogid in ( rlid );

        	update tblresponselogs set resurvey=1, responsestatusid = 3, rejectreason = $8 where responselogid in ( rlid );
        	...
        	from tblsurveydetaails where responselogid in ( rlid ) and surveydetailsid=maxsdid returning resurveydetailsid into resid;
            ...
            where activityid=$2 and responsedate is not null and response<>'' and value<>'' and  responselogid in ( rlid ) ;
            ...
            select  distinct value,responselogid,categorypropertyallocationid,"valueId" valueid
                          ...
                          where activityid=$2 and responsedate is not null and response<>'' and value<>'' and responselogid in ( rlid ) and value<>'{"mobile_number":null,"validated":null}'
            )
            ,cteas as (
